...
Titanic <- read.table(file = "Titanic.csv", #Name of text file.
           ...
           stringsAsFactors = FALSE)         #¿convert string to factors? NO EN ESTE CASO NO TIENE SENTIDO (ej:nombres)
#See structure 
str(Titanic)
...
unique(Titanic$PClass) #unique function returns a vector with duplicate elements removed (da los diferentes valores que hay) 
...
#IMP-Cambiar nombres de las clases y factores
...
str(Titanic)
#Another useful function when having a variable with 0 and 1 is ifelse() = otra forma de hacerlo(da igual)
...
# Define a new dataset only with correctly observed data - PERO PERDEMOS MAZO OBS (en este caso)
...
#A veces es mejor eliminar una variable con mazo NAs si no nos hace falta
